...
const cards = computed(() => !data.value ? [] : [
  { label: 'Mes projets', value: data.value.mes_projets, icon: 'i-lucide-folder-kanban', color: 'brand' },
  { label: 'Mes tâches', value: data.value.mes_taches, icon: 'i-lucide-check-square', color: 'accent' },
  { label: 'Documents', value: data.value.documents, icon: 'i-lucide-file', color: 'success' },
...
<template>
  <div>
    ...
    <div v-else-if="data">
      <p class="text-xs font-semibold uppercase tracking-wide text-accent mb-1">Vue d'ensemble</p>
