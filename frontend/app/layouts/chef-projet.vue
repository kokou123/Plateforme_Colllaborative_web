...
const links = [
  { label: 'Tableau de bord', to: '/chef-projet/dashboard', icon: 'i-lucide-layout-dashboard' },
  { label: 'Projets', to: '/chef-projet/projets', icon: 'i-lucide-folder-kanban' },
  { label: 'Tâches', to: '/chef-projet/taches', icon: 'i-lucide-check-square' },
  { label: 'Documents', to: '/chef-projet/documents', icon: 'i-lucide-folder' },
  { label: 'Assistant IA', to: '/chef-projet/assistant', icon: 'i-lucide-sparkles' },
]
</script>
      ...
      <header class="h-16 bg-ink-800 border-b border-ink-700 flex items-center justify-between px-6 gap-4">
        ...
        <div class="relative">
          <button @click="userMenuOpen = !userMenuOpen" class="flex items-center gap-2">
            <div class="w-9 h-9 rounded-full bg-accent text-white flex items-center justify-center text-sm font-medium font-display">
          ...
          <div v-if="userMenuOpen" class="absolute right-0 top-11 bg-ink-800 border border-ink-700 rounded-lg shadow-xl py-1.5 w-44 z-10">
            <div class="px-3 py-2 border-b border-ink-700">
              ...
              <p class="text-xs text-mist">Chef de projet</p>
            </div>
